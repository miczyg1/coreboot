	...
	{0xc4, "SCH5545", {
		...
		{NOLDN, "Global Configuration",
			{0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,0x09,
			 0x0a,0x0b,0x0c,0x0d,0x0e,0x0f,0x10,0x11,0x12,0x13,
			 0x14,0x15,0x16,0x17,0x18,0x19,0x1a,0x1b,0x1c,0x1d,
			 0x1e,0x1f,0x20,0x21,0x22,0x23,0x24,0x26,0x27,0x28,
			 0x2a,0x2b,0x2c,0x2d,0x2e,0x2f,EOT},
			{RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,MISC,RSVD,RSVD,
			 RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,
			 RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,
			 RSVD,RSVD,0xc6,MISC,0x04,0x04,0x00,RSVD,RSVD,RSVD,
			 RSVD,RSVD,RSVD,RSVD,RSVD,RSVD,EOT}},
		{0x1, "Keyboard",
			{0x30,0xf0,0xf1,EOT},
			{0x00,0x00,0x00,EOT}},
		{0x7, "COM1",
		...
		{0xb, "Floppy",
			...
			{0x00,0x0e,0x00,0xff,RSVD,0x00,RSVD,EOT}},
		{0xc, "LPC Interface",
			{0x30,0x40,0x41,0x42,0x43,0x44,0x45,0x46,0x47,0x48,
			 ...
			 0x71,0x72,0x73,0x78,0x79,0x7a,0x7b,0x7c,0x7d,0x7e,
			 0x7f,0x80,0x81,0x82,0x83,EOT},
			{0x00,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
			 0xff,0xff,0xff,0xff,0xff,0xff,0xff,0x00,0x00,0x00,
			 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
			 0x00,0x00,0x00,0x01,0x0c,0x2e,0x00,0x0f,0x00,0x00,
			 0x00,0x07,0x07,0x00,0x00,0x07,0x08,0x00,0x00,0x3f,
			 0x0a,0x00,0x00,0x00,0x00,0x60,0x00,0x00,0x00,0x00,
			 0x00,0x00,0x80,0x78,0x08,EOT}},
		{0x11, "Parallel port",
			{0x30,0xf0,0xf1,EOT},
			{0x00,0x3c,0x00,EOT}},
		{EOT}}},
...
static void print_ec_datareg(uint16_t port, uint16_t reg)
{
	/* 2 bytes of address  and 4 bytes of data */
	OUTB((reg & 0xfc), port);
	OUTB((reg >> 8), port + 1);
	printf("%02x %02x %02x %02x ", INB(port + 2), INB(port + 3),
				       INB(port + 4), INB(port + 5));
}

static void smsc_dump_ec(uint16_t ec_base, uint32_t length)
{
	uint32_t i;
	...
	for(i = 0; i < length; i += 4) {
		...
		print_ec_datareg(ec_base, (i & 0xffff));
	}
}
				     ...
				     uint8_t revreg)
{
	...
	if (extra_dump) {
		...
		case 0xc4: /* SMSC5545 */
			...
			if (runtime_base) {
				printf("Runtime Register Block:\n");
				dump_io(runtime_base, 0x35);
			} else
				printf("Runtime Register Block not mapped on this Super I/O.\n");

			ec_base = regval(port, 0x67) << 8;
			ec_base |= regval(port, 0x66);
			if (ec_base) {
				printf("EC IO registers:\n");
				dump_io(ec_base, 0x0d);
				ec_base += 2; /* address reg at offset 0x2h */
				printf("EC Memory (DCCM):");
				smsc_dump_ec(ec_base, 0xffff); /* up to 64KB */
			} else
				printf("EC Register Block not mapped on this Super I/O.\n");
			break;
		default:
